...
/*   Updated: 2024/09/02 10:45:11 by olamrabt         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
mlx_image_t *ft_get_frame(t_data *data)
{
	...
	if (direction <= 0)
		path = gc_strjoin("./animation/l", gc_itoa(data->frame_num, &data->addr), &data->addr);
	else
		path = gc_strjoin("./animation/r", gc_itoa(data->frame_num, &data->addr), &data->addr);
